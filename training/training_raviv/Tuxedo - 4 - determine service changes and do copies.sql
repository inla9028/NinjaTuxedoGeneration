...
**  +  '29.0.0.0'
**  +  'Added as part of Fokus 29.0.0.0'
*/
...
where ninja_version = '29.0.0.0' -- ie current prod version
...
from tux_services_versions where ninja_version = '29.0.0.0' -- ie current prod version
--from tux_services
group by in_use;
...
from tux_services_versions a where ninja_version = '29.0.0.0' -- ie current prod version
--from tux_services a where 1 = 1 -- ie current prod version
  ...
  and (exists (select 1
               ...
               where b.ninja_version = '29.0.0.0' -- ie current prod version
       ...
       (--(select count(*) from tux_fml_buffers d     where d.svc_name = a.svc_name) <>
        (select count(*) from tux_fml_buffers_versions d where d.ninja_version = '29.0.0.0' and d.svc_name = a.svc_name) <>
...
select svc_name, null as in_use, null as conversational, 
       ...
       'Added as part of Fokus 29.0.0.0' as comments -- *** MODIFY THIS EACH TIME ***
-- select svc_name, count(*)
from tux_fml_buffers_new a
where not exists (select 1
                    --from tux_services_versions b where ninja_version = '29.0.0.0'
...
select a.svc_name, '     ' || c.in_use as in_use, buffer_type, field_name, field_seq, field_type, field_size, field_max_occurence--, field_def_value, exclude_from_csapiban_copy, csapiban_mapping1, csapiban_mapping2, csapiban_mapping3, a.comments
       ...
       --a.svc_name, count(*)
from tux_fml_buffers_new a, 
     tux_services_versions c where c.ninja_version = '29.0.0.0' -- ie current prod version' 
...
select * 
from tux_fml_buffers_versions a where ninja_version = '29.0.0.0' -- ie current prod version' 
...
from tux_fml_buffers_new new_flds, 
     ...
     tux_fml_buffers_versions old_flds where old_flds.ninja_version = '29.0.0.0' -- ie current prod version' 
